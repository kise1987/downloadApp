...
if (!fs.existsSync(dirName)) {
    console.error(dirName + "文件夹不存在，请重新制定根文件夹！");
    process.exit();
}
...
function downloadFile(file, req, res) {
    fs.readFile(filename, 'binary', function(err, file) {
        var contentType = 'application/force-download';/*mime.lookupExtension(path.extname(filename))*/;
        res.writeHead(200, {
...
http.createServer(function(req, res) {
    ...
    if (pathname == '/') {
        listDirectory(dirName, req, res);
    } else {
        filename = path.join(dirName, pathname);
